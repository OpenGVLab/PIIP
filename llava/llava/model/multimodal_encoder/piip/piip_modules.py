...
from .clip_hf_wrapper import CLIPLayerWrapper
from .convnext_timm_wrapper import ConvNextTimmLayerWrapper


def check_finite(*tensors):
    if os.getenv("CHECK_FINITE", "1") == "0":
        print("!!! check finite disabled")
        return True
...
class ReshapeAndLN(nn.Module):
    def __init__(self):
        super(ReshapeAndLN, self).__init__()
    def forward(self, x):
        #(B, H, W, C)
        #->(B,N,C)
        B, H, W, C = x.shape
        reshaped= x.reshape(B, H*W, C)
        #layer normalization in the last dimension
        ln_x= F.layer_norm(reshaped, reshaped.shape[-1:])
        # reshape back
        ln_x = ln_x.reshape(B, H, W, C)
        # ->(B, C, H, W)
        return ln_x.permute(0, 3, 1, 2)
...
def deform_inputs_1(x1, x2, patch_size1=16, patch_size2=16):
    # query = small image
    # key = large image
    # x1 is small image
    _, _, h1, w1 = x1.shape
...
def deform_inputs_2(x1, x2, patch_size1=16, patch_size2=16):
    # query = large image
    # key = small image
    # x1 is large image
    _, _, h1, w1 = x1.shape
...
class Injector(nn.Module):
    def __init__(self, dim, num_heads=6, n_points=4, n_levels=1, deform_ratio=1.0,
        ...
        self.ca_weight = nn.Parameter(0. * torch.ones((dim)), requires_grad=True)
        self.cffn_weight = nn.Parameter(0. * torch.ones((dim)), requires_grad=True)
    ...
    def forward(self, query, reference_points, feat, spatial_shapes, level_start_index, H, W):
        ...
        def _inner_forward(query, feat):
            ...
            query = query + self.ca_weight * attn
            if self.with_cffn:
                query = query + self.cffn_weight * self.drop_path(self.ffn(self.ffn_norm(query), H, W))
...
def forward_blocks(x, H, W, blocks, cls_=None, return_hw=False):
    if len(blocks) == 0:
        return x, cls_
    
    
    if isinstance(blocks[0], ConvNextTimmLayerWrapper):
        assert cls_ is None
        for _, blk in enumerate(blocks):
            x, H, W = blk(x, H, W)
    
    elif isinstance(blocks[0], CLIPLayerWrapper):
        if cls_ is not None:
        ...
        raise NotImplementedError(type(blocks[0]))
...
class BidirectionalInteractionUnit(nn.Module):
    def __init__(self, branch1_dim, branch2_dim, branch1_img_size, branch2_img_size, 
                 branch1_patch_size, branch2_patch_size, 
        ...
        super().__init__()
        self.attn_type = attn_type
        self.branch1_patch_size = branch1_patch_size
        self.branch2_patch_size = branch2_patch_size
...
class ThreeBranchInteractionBlock(nn.Module):
    def __init__(self, branch1_dim, branch2_dim, branch3_dim, 
                 ...
                 branch1_patch_size, branch2_patch_size, branch3_patch_size, 
                 attn_type='deform', **kwargs):
        super().__init__()
        self.attn_type = attn_type

        self.interaction_units_12 = BidirectionalInteractionUnit(branch1_dim, branch2_dim, branch1_img_size, branch2_img_size, branch1_patch_size, branch2_patch_size, attn_type=attn_type, **kwargs)
        self.interaction_units_23 = BidirectionalInteractionUnit(branch2_dim, branch3_dim, branch2_img_size, branch3_img_size, branch2_patch_size, branch3_patch_size, attn_type=attn_type, **kwargs)
    ...
    def forward(self, x1, x2, x3, branch1_blocks, branch2_blocks, branch3_blocks, 
        ...
        x1, cls1 = forward_blocks(x1, H1, W1, branch1_blocks, cls1)
        x2, cls2 = forward_blocks(x2, H2, W2, branch2_blocks, cls2)
        x3, cls3 = forward_blocks(x3, H3, W3, branch3_blocks, cls3)
...
class TwoBranchInteractionBlockWithHWOutPut(nn.Module):
    def __init__(self, branch1_dim, branch2_dim,
                 branch1_img_size, branch2_img_size,
                 attn_type='deform', **kwargs):
        super().__init__()
    ...
    def forward(self, x1, x2, branch1_blocks, branch2_blocks,
        ...
        x1, cls1, H1, W1 = forward_blocks(x1, H1, W1, branch1_blocks, cls1, return_hw=True)
        x2, cls2, H2, W2 = forward_blocks(x2, H2, W2, branch2_blocks, cls2, return_hw=True)
...
class ThreeBranchInteractionBlockWithHWOutPut(nn.Module):
    def __init__(self, branch1_dim, branch2_dim, branch3_dim, 
                 branch1_img_size, branch2_img_size, branch3_img_size, 
                 branch1_patch_size, branch2_patch_size, branch3_patch_size, 
                 attn_type='deform', **kwargs):
    ...
    def forward(self, x1, x2, x3, branch1_blocks, branch2_blocks, branch3_blocks, 
        ...
        x1, cls1, H1, W1 = forward_blocks(x1, H1, W1, branch1_blocks, cls1, return_hw=True)
        x2, cls2, H2, W2 = forward_blocks(x2, H2, W2, branch2_blocks, cls2, return_hw=True)
        x3, cls3, H3, W3 = forward_blocks(x3, H3, W3, branch3_blocks, cls3, return_hw=True)
