...
class CustomCallback(TrainerCallback):
    ...
    def on_log(self, args, state, control, **kwargs):
        ...
        print(f"Step {str(current_step).rjust(len(str(total_steps)))}/{total_steps}, Elasped time {format_seconds(elapsed_time)}, ETA {format_seconds(remaining_time)}")
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer,
    ...
    if getattr(trainer.args, "tune_mm_mlp_adapter", False):
        ...
        weight_to_save.update(get_mm_adapter_state_maybe_zero_3(trainer.model.named_buffers(), keys_to_match))
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        for i in instances:
            if 'error' in i and i['error']:
                self.err_cnt += 1
                print("Error count", self.err_cnt)
                
        input_ids, labels = tuple([instance[key] for instance in instances if not ('error' in instance and instance['error'])]
        ...
        if 'image' in instances[0]:
            images = [instance['image'] for instance in instances if not ('error' in instance and instance['error'])]
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        # 初始化视觉模型
        model_args.output_dir = model.config.output_dir = training_args.output_dir
        ...
        model.config.vision_tower_name = model_args.vision_tower
        ...
        if model_args.tune_mm_mlp_adapter or training_args.freeze_llm:
            ...
            # pretrain时冻住模型，放开projector
            model.requires_grad_(False)
            for p in model.get_model().mm_projector.parameters():
                p.requires_grad = True
        
        if not model_args.tune_mm_mlp_adapter:
            model.get_vision_tower().unfreeze_all()
    ...
    for k, v in model.named_parameters():
        if v.requires_grad:
            unfreeze_params.append(k)
        else:
            freeze_params.append(k)
                
    with open(os.path.join(training_args.output_dir, "unfreeze_params.txt"), "w") as f:
        print("\n".join(unfreeze_params), file=f)
    with open(os.path.join(training_args.output_dir, "freeze_params.txt"), "w") as f:
        print("\n".join(freeze_params), file=f)
