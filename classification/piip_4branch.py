...
from piip_modules import deform_inputs_1, deform_inputs_2, Permute, FourBranchInteractionBlock
...
class PIIPFourBranch(nn.Module):
    def __init__(self, n_points=4, deform_num_heads=6,
        ...
        self.branch4_model_type = branch4.pop("model_type", "deit")
        ...
        self.branch4 = self.create_branch(self.branch4_model_type, branch4)

        assert len(self.branch1_interaction_indexes) == len(self.branch2_interaction_indexes) == len(self.branch3_interaction_indexes) == len(self.branch4_interaction_indexes)
        self.interactions = nn.Sequential(*[
            FourBranchInteractionBlock(
                ...
                branch4_img_size=self.branch4.img_size,
    ...
    def forward(self, x):
        ...
        if self.interact_attn_type == "deform":
            ...
            deform_inputs["4to3"] = deform_inputs_1(x3, scale_factor_3to4)
            deform_inputs["3to4"] = deform_inputs_2(x4, scale_factor_3to4)
        else:
        ...
        x4, cls4, bs4, n4, dim4, H4, W4 = self.forward_embedding(self.branch4, x4, self.branch4_model_type)
        ...
        for i, layer in enumerate(self.interactions):
            ...
            x1, x2, x3, x4, cls1, cls2, cls3, cls4 = layer(x1, x2, x3, x4,
        ...
        if self.separate_head:
            assert self.branch1.cls_token is not None and self.branch2.cls_token is not None and self.branch3.cls_token is not None and self.branch4.cls_token is not None
            ...
            x4 = self.branch4.head(self.branch4.norm(x4)[:, 0])
        ...
        out = out.reshape(bs1, dim1, H4*W4).transpose(1,2)
