...
class InternViT6B(nn.Module):
    def __init__(self, in_chans=3, patch_size=16, pretrain_patch_size=16, img_size=224, pretrain_img_size=224, 
                 qkv_bias=False, 
                 drop_path_rate=0.0,
                 embed_dim=3200, num_heads=25, mlp_ratio=4, init_values=None, qk_normalization=False, depth=48,
                 use_flash_attn=True, with_cp=True, layerscale_force_fp32=False, 
                 window_attn=False, window_size=14,
                 pretrained=None, norm_layer_type='ln', 
        ...
        if use_flash_attn and not has_flash_attn:
            raise ValueError('Flash Attention is not available. Please install flash attention or set use_flash_attn to False.')
        ...
        print(f'window attention: {window_attn}')
        ...
        if use_final_norm_and_head:
            self.norm = self.norm_layer_for_blocks(embed_dim)
    ...
    def init_weights(self, pretrained=None):
        ...
        if isinstance(pretrained, str):
            ...
            if os.environ.get("STATE_DICT_STRICT") == "False":
                # for flops calculation
                message = self.load_state_dict(checkpoint, strict=False)
            else:
                # Note: if head weights are missing, please download official augreg weights (.npz) and convert to .pth according to classification readme.
                message = self.load_state_dict(checkpoint, strict=True)
                
            print("msg for loading checkpoints", message)
        else:
