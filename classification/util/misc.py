...
class MetricLogger(object):
    ...
    def log_every(self, iterable, print_freq, header=None):
        ...
        for obj in iterable:
            ...
            if i % print_freq == 0 or i == len(iterable) - 1 or i == 50: # or i == 20
                eta_seconds = iter_time.global_avg * (len(iterable) - i)
                eta_string = str(datetime.timedelta(seconds=int(eta_seconds)))
                epoch_time = round(iter_time.avg * len(iterable) / 60)
                if torch.cuda.is_available():
...
def init_distributed_mode(args):
    if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:
        ...
        if hasattr(args, 'port'):
            ...
            if _is_free_port(29506):
                os.environ['MASTER_PORT'] = '29506'
            else:
                free_port = str(find_free_port())
                os.environ['MASTER_PORT'] = free_port
                print("master port:", free_port)
...
def count_params(*modules):
    ...
    return f"{round(n_params / 10**6, 2)}M"
