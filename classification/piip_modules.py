...
from deit import Layer_scale_init_Block as DeitLSBlock
from intern_vit_6b import Block as InternViT6BBlock
...
def deform_inputs_1(x, scale_ratio=1.0, patch_size1=16, patch_size2=16):
    # query = small image
    # key = large image
    # x is small image
    bs, c, h, w = x.shape
    h_ = int(h / scale_ratio)
    w_ = int(w / scale_ratio)
    spatial_shapes = torch.as_tensor([(math.ceil(h_ / patch_size2), math.ceil(w_ / patch_size2))], 
                                     dtype=torch.long, device=x.device)
    level_start_index = torch.cat((spatial_shapes.new_zeros(
        (1,)), spatial_shapes.prod(1).cumsum(0)[:-1]))
    reference_points = get_reference_points([(math.ceil(h / patch_size1), math.ceil(w / patch_size1))], x.device)
    deform_inputs1 = [reference_points, spatial_shapes, level_start_index]
    
    return deform_inputs1


def deform_inputs_2(x, scale_ratio=1.0, patch_size1=16, patch_size2=16):
    # query = large image
    # key = small image
    # x is large image
    bs, c, h, w = x.shape
    h_ = int(h * scale_ratio)
    w_ = int(w * scale_ratio)
    spatial_shapes = torch.as_tensor([(math.ceil(h_ / patch_size1), math.ceil(w_ / patch_size1))], 
                                     dtype=torch.long, device=x.device)
    level_start_index = torch.cat((spatial_shapes.new_zeros(
        (1,)), spatial_shapes.prod(1).cumsum(0)[:-1]))
    reference_points = get_reference_points([(math.ceil(h / patch_size2), math.ceil(w / patch_size2))], x.device)
    ...
    def forward(self, query, reference_points, feat, spatial_shapes, level_start_index, H, W):
        ...
        def _inner_forward(query, feat):
            ...
            with torch.cuda.amp.autocast(False):
                query = query.float() + self.ca_gamma * attn.float()
            
            if self.with_cffn:
                with torch.cuda.amp.autocast(False):
                    query = query.float() + self.cffn_gamma * self.drop_path(self.ffn(self.ffn_norm(query), H, W)).float()
...
def forward_blocks(x, H, W, blocks, cls_=None):
    ...
    elif isinstance(blocks[0], DeitLSBlock) or isinstance(blocks[0], InternViT6BBlock):
        if cls_ is not None:
...
class FourBranchInteractionBlock(nn.Module):
    ...
    def forward(self, x1, x2, x3, x4, branch1_blocks, branch2_blocks, branch3_blocks, branch4_blocks,
        ...
        x4, cls4 = forward_blocks(x4, H4, W4, branch4_blocks, cls4)
