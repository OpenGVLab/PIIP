...
try:
    ...
    from mmcv.ops.multi_scale_deform_attn_optimized import MultiScaleDeformableAttentionOptimized
    ...
    from mmcv.cnn.bricks.transformer import MultiScaleDeformableAttentionOptimized
    ...
    def init_weights(self):
        ...
        for m in self.modules():
            if isinstance(m, MultiScaleDeformableAttention):
                m.init_weights()
            if isinstance(m, MultiScaleDeformableAttentionOptimized):
                m.init_weights()
        if not self.as_two_stage:
            xavier_init(self.reference_points, distribution='uniform', bias=0.)
        normal_(self.level_embeds)

    @force_fp32(apply_to=('memory'))
...
class DinoTransformerDecoder(DeformableDetrTransformerDecoder):
    ...
    def forward(self,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            query_pos = self.ref_point_head(query_sine_embed.to(query.dtype))
            ...
            if reg_branches is not None:
                tmp = reg_branches[lid](output.to(torch.float32))
                assert reference_points.shape[-1] == 4
                # TODO: should do earlier
                new_reference_points = tmp + inverse_sigmoid(
                    reference_points.float(), eps=1e-3)
                new_reference_points = new_reference_points.sigmoid()
                reference_points = new_reference_points.detach()
...
class DinoTransformer(DeformableDetrTransformer):
    ...
    def forward(self,
        ...
        output_memory, output_proposals = self.gen_encoder_output_proposals(
            memory, mask_flatten, spatial_shapes) # torch.float16, torch.float32
        output_memory = output_memory.float()
        enc_outputs_class = cls_branches[self.decoder.num_layers](
            output_memory)
        ...
        if dn_label_query is not None:
            query = torch.cat([dn_label_query.to(query.dtype), query], dim=1)
        if dn_bbox_query is not None:
            reference_points = torch.cat([dn_bbox_query.to(topk_coords_unact.dtype),
                                          topk_coords_unact], dim=1)
        else:
