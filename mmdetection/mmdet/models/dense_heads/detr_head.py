...
class DETRHead(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if hasattr(self.train_cfg, 'fed_loss_cfg'):  # default
            from detectron2.data.detection_utils import get_fed_loss_cls_weights
            dataset_names = self.train_cfg.fed_loss_cfg.dataset_names
            freq_weight_power = self.train_cfg.fed_loss_cfg.freq_weight_power
            if dataset_names == 'coco':
                fed_loss_cls_weights = torch.tensor([64115, 3252, 12251, 3502, 2986, 3952, 3588, 6127, 3025, 4139,
                                                     1711, 1734, 705, 5570, 3237, 4114, 4385, 2941, 1529, 1968,
                                                     2143, 960, 1916, 2546, 5528, 3968, 6841, 3810, 2402, 2184,
                                                     3082, 1654, 4262, 2261, 2506, 2629, 3476, 3486, 3394, 8501,
                                                     2533, 9189, 3555, 4326, 3529, 7111, 2243, 1586, 2365, 1699,
                                                     1939, 1683, 1222, 3166, 1523, 2925, 12774, 4423, 4452, 3682,
                                                     11837, 3353, 4561, 3524, 1876, 3076, 2115, 4803, 1547, 2877,
                                                     217, 4678, 2360, 5332, 4659, 3593, 947, 2140, 189, 1007])
                self.fed_loss_cls_weights = fed_loss_cls_weights.float() ** freq_weight_power
            else:
                self.fed_loss_cls_weights = get_fed_loss_cls_weights(dataset_names=dataset_names,
                                                                     freq_weight_power=freq_weight_power)
            logging.info("init fed_loss_cls_weights!")
    ...
    def loss_single(self,
        ...
        (labels_list, label_weights_list, bbox_targets_list, bbox_weights_list,
         num_total_pos_per_gpu, num_total_neg_per_gpu) = cls_reg_targets

        # TODO: use class_weight to generate label_weights_list
        ...
        num_total_pos = cls_scores.new_tensor([num_total_pos_per_gpu])
        num_total_pos = torch.clamp(reduce_mean(num_total_pos), min=1).item()
        num_total_neg = num_total_pos_per_gpu + num_total_neg_per_gpu - num_total_pos
        ...
        if self.sync_cls_avg_factor:
            ...
            cls_avg_factor = num_total_pos_per_gpu * 1.0 + \
                num_total_neg_per_gpu * self.bg_cls_weight
        cls_avg_factor = max(cls_avg_factor, 1)

        if not hasattr(self.train_cfg, 'fed_loss_cfg'): # default setting
            loss_cls = self.loss_cls(
                cls_scores, labels, label_weights, avg_factor=cls_avg_factor)
        else:
            loss_cls = self.loss_cls( # [9900, 1203]
                cls_scores, labels, label_weights, avg_factor=None, reduction_override="none")
            N, K = loss_cls.size(0), loss_cls.size(1)
            fed_loss_num_classes = self.train_cfg.fed_loss_cfg.fed_loss_num_classes
            ...
            loss_cls = (loss_cls * weight).sum() / (cls_avg_factor + eps)
            # print(loss_cls_old, loss_cls)
    ...
    def _get_target_single(self,
        ...
        if hasattr(self.train_cfg, 'reweight_cfg'):
            class_weight = self.train_cfg.reweight_cfg.class_weight + [1.0]
            class_weight = np.array(class_weight)
            class_weight = torch.from_numpy(class_weight).cuda()
            class_weight = class_weight[labels]
            label_weights = label_weights * class_weight
            bbox_weights = bbox_weights * class_weight[:, None]
