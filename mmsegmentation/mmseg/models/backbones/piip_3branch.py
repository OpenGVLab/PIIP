...
class PIIPThreeBranch(nn.Module):
    ...
    def forward(self, x):
        ...
        scale_factor_3to2 = self.branch3_real_size / self.branch2_real_size
        if scale_factor_3to2 > 1:
            x3 = F.interpolate(x, scale_factor=scale_factor_3to2, mode='bilinear', align_corners=False)
        else:
            x3 = x.clone()
